...
export default function Header() {
  ...
  return (
    <header className={`fixed inset-x-0 top-0 z-40 transition-colors duration-300 ${solid ? 'bg-white/90 shadow-soft backdrop-blur' : 'bg-transparent'}`}>
      <div className="container-x flex h-16 items-center justify-between">
        ...
        <nav className="hidden items-center gap-7 md:flex">
          {NAV.map((n) => (
            <a key={n.href} href={n.href} className={`text-[15px] font-medium opacity-90 transition hover:opacity-100 ${tone}`}>{n.label}</a>
          ))}
